...
def post_drinks():
     ...
     drink = Drink(id= new_id,title=new_title, recipe=x)
     drink.insert()

     drinksRes = Drink.query.filter(Drink.id == new_id).one_or_none()

     if drinksRes == None:
        abort(404)

     return  jsonify({'success':True, 'drinks':Drink.long(drinksRes)})
...
def patch_drinks(id):
     ...
     drinksRes = Drink.query.filter(Drink.id == id).one_or_none()
